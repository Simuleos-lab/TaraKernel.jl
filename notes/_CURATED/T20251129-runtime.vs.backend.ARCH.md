# RunTime vs BackEnd Representation

## Kernel Boundaries and Canonical Bytes
...
- General workflows will be:
    - `BackEnd.Storage` -> `[Interface.read]` -> `canonical.data.bytes` -> `[Kernel.parse]` -> `Kernel.objects` 
    - `Kernel.objects` -> `[Kernel.serialize]` -> `canonical.data.bytes` -> `[Interface.write]` -> `BackEnd.Storage` 
...
- Must canonical bytes of a requested Tape segment
- Must store the canonical bytes of a target Tape segment
