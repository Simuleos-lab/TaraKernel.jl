...
- #LAW
- The kernel recognizes **Storage** as the authoritative source of truth for content it has committed.
- Storage is not “best effort” persistence.
- Storage is the definition of what has been recorded by the kernel.
- Corrupted or invalid bytes may physically exist at Storage
- Validation is a permanent obligation of the kernel.
- It is a kernel violation to treat invalid data as valid Storage content.
    - No unvalidated record may be committed.
    - No unchecked structure may be accepted under kernel authority as valid.
    - No unverifiable byte sequence may be treated as valid data by the kernel.
- Storage content is required to validate under kernel invariants whenever it is accessed:
    - canonical encoding,
    - content hashing,
    - commit verification,
    - structural integrity.
- Validation is not a write-time convenience.
- Any of the following conditions invalidate Storage content under kernel authority:
    - detected corruption,
    - hash mismatches,
    - invalid commits,
    - illegal rewriting,
...
- Storage is authoritative not because it is correct by assumption,
    - but because correctness is enforced by the kernel.
- Storage does not represent a belief.
- It represents **fact as recorded**.
- Storage is not:
    - a cache,
    - a materialized view,
    - a denormalized projection,
    - or an optimization.
- Storage is the historical ground truth of the system as recognized by the kernel.
- Runtime may speculate.
- Runtime may be wrong.
- Storage content treated as valid by the kernel MUST NOT be.
- If Runtime contradicts valid Storage: Runtime is wrong.
- If Storage content contradicts itself under kernel validation, either: 
    - the kernel is in error.
